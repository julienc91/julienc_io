...
test.describe("About page", () => {
  test("should load successfully", async ({ page }) => {
    ...
    await expect(page).toHaveTitle(
      /À propos | Julien Chaumont - Software Engineer/,
    )
    ...
    const projectItems = page.locator(
      'section:has(h2:text("Réalisations")) .item',
    )
    expect(await projectItems.count()).toBeGreaterThan(0)

    // Check if there are education items
    const educationItems = page.locator(
      'section:has(h2:text("Formation")) .item',
    )
    expect(await educationItems.count()).toBeGreaterThan(0)

    // Check if the legal section has content
    const legalContent = page.locator(
      'section:has(h2:text("Mentions légales")) p',
    )
